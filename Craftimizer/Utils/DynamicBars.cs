...
internal static class DynamicBars
{
    ...
    private static ImRaii2.Disposable PushCollectableColor(this in BarData bar, float collectability, bool colorUnmetThreshold = true)
    {
        if (bar.Collectability is not { } collectabilities)
            return new ImRaii2.Disposable(null, false);
        ...
        if (ret == 0)
        {
            ...
            {
                ImGui.PushStyleColor(ImGuiCol.Text, Colors.Collectability);
                return new ImRaii2.Disposable(() => ImGui.PopStyleColor(), true);
            }
            return new ImRaii2.Disposable(null, false);
        }

        ImGui.PushStyleColor(ImGuiCol.Text, Colors.CollectabilityThreshold[ret - 1]);
        return new ImRaii2.Disposable(() => ImGui.PopStyleColor(), true);
    }
